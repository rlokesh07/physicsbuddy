...
const Question = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	$$result.css.add(css);
	return `<div class="question svelte-1yeoynr"><h1 class="svelte-1yeoynr">${escape(question.topic)}</h1> <p class="svelte-1yeoynr"><!-- HTML_TAG_START -->${renderTextWithMathMode(question.question)}<!-- HTML_TAG_END --></p> ${question.image ? `<img${add_attribute('src', question.image, 0)} alt=" Image" class="svelte-1yeoynr">` : ``}</div> <div class="answer">${
		question.options
			? `<form>${each(question?.options, (option, index) => {
				...
				})} <button type="submit" data-svelte-h="svelte-1ou7dtz">Submit</button></form>`
			: ``
	} </div>`;
});
