	...
	return `<div class="fullContainer svelte-jfdexj"><div class="questionBox svelte-jfdexj"><div class="question svelte-jfdexj"><h1 class="svelte-jfdexj">${escape(question.topic)}</h1> <p class="svelte-jfdexj"><!-- HTML_TAG_START -->${renderTextWithMathMode(question.question)}<!-- HTML_TAG_END --></p> ${question.image ? `<img class="questionImage svelte-jfdexj" src="/src/lib/images/HM0003.png" alt=" Image">` : ``}</div> <div class="answer svelte-jfdexj">${
		question.options
			? `<form>${each(question?.options, (option, index) => {
				...
				})} <div class="submit-button-container svelte-jfdexj" data-svelte-h="svelte-kk7jo9"><button type="submit" class="submit-button svelte-jfdexj">Submit</button></div></form>`
			: ``
	}</div></div> </div>`;
});
const canUseDOM = () =>
